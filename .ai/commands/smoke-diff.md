---
description: Smoke-test the diff on the current branch by proving the change actually WORKS, not just that the repo still compiles. Reads the branch diff, surfaces changed exports as probe targets, has you write one targeted behavioral probe, then runs that probe FIRST plus the baseline gates (format, lint, types, related tests, design) and reports pass/fail.
...
logic passes all four cleanly. So this command puts a **targeted behavioral probe first** and runs the
...
code with representative inputs and asserts the intended behavior** — including the edge case the change
was for.
...
A probe is not a placeholder. Assert the behavior the change was _for_. For a Convex handler, exercise
...
- **The probe is untracked on purpose.** If the behavior deserves a permanent test, write one in
