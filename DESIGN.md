...
**What the rule deliberately does not enforce** (both are `/noted-review` judgment calls, not static ones):
...
If you add a new orphan-token warning that _isn't_ a dark counterpart or a documented shadcn primitive, that's a real finding — wire it through to a component or remove it.
